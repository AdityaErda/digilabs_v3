...
class M_kurva extends CI_Model{
    /* GET */
  ...
  public function getKurva($data = null){
    if (isset($data['kurva_id'])) $this->db->where('kurva_id', $data['kurva_id']);

    $this->db->select('*');
    $this->db->from('sample.sample_kurva a');
    $this->db->join('sample.sample_perhitungan_sample b', 'b.rumus_id = a.id_rumus', 'left');
    $this->db->join('sample.sample_jenis c', 'c.jenis_id = b.jenis_id', 'left');
    // $this->db->join('sample.sample_kurva_header d', 'd.id_kurva = a.kurva_id', 'right');


    $sql = $this->db->get();

    return (isset($data['kurva_id'])) ? $sql->row_array() : $sql->result_array();
  }

  public function getKurvaHeader($data = null){
    if (isset($data['kurva_header_id'])) $this->db->where('kurva_header_id', $data['kurva_header_id']);
    if (isset($data['id_kurva'])) $this->db->where('id_kurva', $data['id_kurva']);

    $this->db->select('*');
    $this->db->from('sample.sample_kurva_header a');
    $this->db->order_by('kurva_header_urut', 'asc');

    $sql = $this->db->get();

      if ($sql) {
          return (isset($data['kurva_header_id'])) ? $sql->row_array() : $sql->result_array();
      } else {
          return false;
    }
  }

  public function getKurvaIsi($data = null){
    if (isset($data['kurva_isi_id'])) $this->db->where('kurva_isi_id', $data['kurva_isi_id']);
    if (isset($data['id_kurva'])) $this->db->where('id_kurva', $data['id_kurva']);
    if (isset($data['id_kurva_header'])) $this->db->where('id_kurva_header', $data['id_kurva_header']);

    $this->db->select('*');
    $this->db->from('sample.sample_kurva_isi a');
    $this->db->order_by('kurva_urut', 'asc');

    $sql = $this->db->get();

      if ($sql) {
          return (isset($data['kurva_isi_id'])) ? $sql->row_array() : $sql->result_array();
      } else {
          return false;
    }
  }
    ...
    /* Insert */
  public function insertKurva($data){
    $this->db->insert('sample.sample_kurva', $data);
    return $this->db->affected_rows();
  }

  public function insertKurvaHeader($data){
    $this->db->insert('sample.sample_kurva_header', $data);
    return $this->db->affected_rows();
  }

  public function insertKurvaIsi($data){
    $this->db->insert('sample.sample_kurva_isi', $data);
    return $this->db->affected_rows();
  }
    /* Insert */

    /* Update */
  public function updateKurva($id, $data){
    $this->db->where('kurva_id', $id);
    $this->db->update('sample.sample_kurva', $data);
    return $this->db->affected_rows();
  }

  public function updateKurvaHeader($id, $data){
    $this->db->where('kurva_header_id', $id);
    $this->db->update('sample.sample_kurva_header', $data);
    return $this->db->affected_rows();
  }

  public function updateKurvaIsi($id, $data){
    $this->db->where('kurva_isi_id', $id);
    $this->db->update('sample.sample_kurva_isi', $data);
    return $this->db->affected_rows();
  }
  ...
  public function updateRumusMultiple($data, $id){
    ...
    /* Delete */
  public function deleteKurva($id){
    $this->db->where('kurva_id', $id);
    $this->db->delete('sample.sample_kurva');
    return $this->db->affected_rows();
  }

  public function deleteKurvaHeader($id){
    $this->db->where('kurva_header_id', $id);
    $this->db->delete('sample.sample_kurva_header');
    return $this->db->affected_rows();
  }

  public function deleteKurvaIsi($id){
    $this->db->where('kurva_isi_id', $id);
    $this->db->delete('sample.sample_kurva_isi');
    return $this->db->affected_rows();
  }
