<script type="text/javascript">
  $(function() {
    ...
    $('#table thead tr').clone(true).addClass('filters').appendTo('#table thead');
      ...
      "ajax": {
        "url": "<?= base_url() ?>/master/jenis_document/getJenisDocument",
        "dataSrc": ""
      },
      "columns": [{
          "data": "jenis_kode"
  ...
  function fun_edit(id) {
    $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
      if(!json.user_id){
        ...
        $.getJSON('<?= base_url('master/jenis_document/getJenisDocument') ?>', {
          jenis_id: id
        }, function(json) {
  ...
  $("#form_modal").on("submit", function(e) {
    ...
    $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
      if(!json.user_id){
        ...
        if ($('#jenis_id').val() != '') var url = '<?= base_url('master/jenis_document/updateJenisDocument') ?>';
        else var url = '<?= base_url('master/jenis_document/insertJenisDocument') ?>';
    ...
    $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
      if(!json.user_id){
        ...
        $.confirmModal('Apakah anda yakin akan menghapusnya?', function(el) {
          $.get('<?= base_url('master/jenis_document/deleteJenisDocument') ?>', {
            jenis_id: id
          }, function(data) {
            $('#close').click();
            toastr.success('Berhasil');
  ...
  function fun_reset() {
    $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
      if(!json.user_id){
        ...
        $.confirmModal('Apakah anda yakin akan reset semua data jenis document ?', function(el) {
          $.get('<?= base_url('master/jenis_document/resetJenisDocument') ?>', {
            jenis_id: 'id'
          }, function(data) {
            $('#close').click();
            toastr.success('Berhasil');
