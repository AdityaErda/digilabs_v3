<script type="text/javascript">
    $(function() {
        ...
        $('#table').DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('master/coba_barang/getCobaBarang') ?>",
                "dataSrc": ""
            },
            "columns": [{
                    "data": "jenis_kode"
    ...
    function fun_edit(id) {
        $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
            if (!json.user_id) {
                ...
                $.getJSON('<?= base_url('master/coba_barang/getCobaBarang') ?>', {
                    jenis_id: id
                }, function(json) {
    ...
    $("#form_modal").on("submit", function(e) {
        ...
        $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
            if (!json.user_id) {
                ...
                if ($('#jenis_id').val() != '') var url = '<?= base_url('master/coba_barang/updateCobaBarang') ?>';
                else var url = '<?= base_url('master/coba_barang/insertCobaBarang') ?>';
        ...
        $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
            if (!json.user_id) {
                ...
                $.confirmModal('Apakah anda yakin akan menghapusnya?', function(el) {
                    $.get('<?= base_url('master/coba_barang/deleteCobaBarang') ?>', {
                        jenis_id: id
                    }, function(data) {
                        $('#close').click();
                        toastr.success('Berhasil');
        ...
        $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
            if (!json.user_id) {
                ...
                $.confirmModal('Apakah anda yakin akan mereset seluruh data jenis barang ?', function(el) {
                    $.get('<?= base_url('master/coba_barang/resetCobaBarang') ?>', {
                        jenis_id: 'id'
                    }, function(data) {
                        $('#close').click();
                        toastr.success('Berhasil');
