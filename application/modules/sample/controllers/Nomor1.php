...
class Nomor extends MY_Controller
{
	...
	public function insertLogsheetMultiple()
	{
		...
		foreach ($data_transaksi as $key_transaksi => $value_transaksi) {
			...
			$param_logsheet['logsheet_jenis'] = $data_jenis['jenis_id'];
			...
			$param_logsheet['logsheet_tipe'] = $value_transaksi['transaksi_tipe'];
			...
			foreach ($data_rumus as $key_rumus => $value_rumus) {
				...
				// $random = $this->input->post('random_temp')[$value_rumus['detail_multiple_rumus_id']];
				...
				// $param_logsheet_detail['logsheet_batasan'] = $valur_rumus['batasan_parameter'];
				...
				foreach ($data_parameter_rumus as $key_parameter_rumus => $value_parameter_rumus) {
					...
					$param_logsheet_detail_detail['rumus_detail_isi'] = $this->input->get_post('logsheet_detail_rumus_isi')[$value_rumus['detail_multiple_rumus_id']][$value_parameter_rumus['detail_parameter_rumus_id']];
	...
	public function insertLogsheetMultipleReview()
	{
		...
		/* 		create docx */
		$templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('./dokumen_dof/template_rutin.docx');
		...
		$pathToSave = './dokumen_dof/' . $this->input->post('transaksi_rutin_id') . '.docx';
	...
	public function insertDOF()
	{
		...
		$tokenUrl = "https://newdevdof.petrokimia-gresik.com/api/Docs/";
		...
		foreach ($dataLogsheet as $key => $value) {
			...
			foreach ($data_logsheet_detail as $k => $val) {
				$hasil[] = array(
					'nomor_sample' => $value['transaksi_nomor'],
					'jenis_uji' => $val['parameter_rumus'],
					'satuan' => $val['rumus_satuan'],
					'hasil_uji' => ':   ' . $val['rumus_hasil'],
				);
			}
		}
		$templateProcessor->cloneBlock('hasil', 0, true, false, $hasil);

		$pathToSave = './dokumen_dof/' . $filenames;
		$templateProcessor->saveAs($pathToSave);
		/* Penciptaan Dokumen DOCX */
		...
		if (isset($item['fileName'])) {
			...
			$user = "digilab";
			$pass = "D3vD1gi2023!!";
			$local_file = FCPATH . './dokumen_dof/' . $filenames;
			...
			$dof_tokenUrl = "https://newdevdof.petrokimia-gresik.com/api/Docs/IsDocExist?filename=" . $filenames;
	...
	public function insertCloseNonLetterDetail()
	{
		$session = $this->session->userdata();

		$param_rutin_detail['id_transaksi_rutin'] = $this->input->post('id_transaksi_rutin');
		$param_rutin_detail['id_transaksi'] = $this->input->post('id_transaksi');
		$param_rutin_detail['when_create'] = date('Y-m-d H:i:s');
		$param_rutin_detail['who_create'] = ($session['role_id'] == '1') ? 'Super Admin' : $session['user_nama_lengkap'];
		$param_rutin_detail['who_seksi_create'] = $session['id_seksi'];
		$param_rutin_detail['transaksi_detail_status'] = '17';

		$this->M_nomor->updateNomorTransaksiDetailSingle($param_rutin_detail);
		...
		$this->M_nomor->updateNomorTransaksiSingle($id, $param_rutin);
