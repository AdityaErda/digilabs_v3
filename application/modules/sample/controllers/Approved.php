...
class Approved extends MY_Controller
{
	...
	// Proccess
	public function procesApproved()
	{
		$isi['judul'] = 'Sample Approved';
		$data = $this->session->userdata();
		$data['id_sidebar'] = $this->input->get('id_sidebar');
		...
		$this->load->view('tampilan/header', $isi);
		$this->load->view('tampilan/sidebar', $data);
		$this->load->view('sample/approved_proces', $result);
		$this->load->view('tampilan/footer');
		$this->load->view('sample/approved_proces_js');
	}
	...
	public function insertApproved()
	{
		...
		sampleLog($data_transaksi['transaksi_id'], null, $id_non_rutin, $data_transaksi['transaksi_tipe'], $data_transaksi['transaksi_status'], 'Pekerjaan Telah Approve Oleh VP Peminta Jasa ');
	}
}
