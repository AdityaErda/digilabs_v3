<script type="text/javascript">
  $(function() {
    ...
    $('#table').DataTable({
      ...
      "ajax": {
        "url": "<?= base_url() ?>sample/lab/getLab?transaksi_status_request=0,n",
        "dataSrc": ""
      },
      "columns": [{
        ...
        },
        {
          "render": function(data, type, full, meta) {
            console.log(full);
            if (full.transaksi_status == '3' && (($('#user_unit_id').val() == 'E44000' && $('#user_poscode').val() == 'E44000000') || $('#user_id').val() == '1')) {
              return '<a class="btn btn-success" href="<?= base_url('sample/lab/procesLab/?header_menu=' . $_GET['header_menu'] . '&menu_id=' . $_GET['menu_id'] . '&non_rutin=') ?>' + full.id_transaksi_non_rutin + '&status=' + full.transaksi_status + '"> Approved </a>'
            } else if (full.transaksi_status == '4' && (( /*$('#user_unit_id').val() == 'E44000' &&*/ full.id_user_disposisi == '<?= $this->session->userdata('user_nik_sap') ?>') || $('#user_id').val() == '1')) {
              return '<a class="btn btn-success" href="<?= base_url('sample/lab/procesLab/?header_menu=' . $_GET['header_menu'] . '&menu_id=' . $_GET['menu_id'] . '&non_rutin=') ?>' + full.id_transaksi_non_rutin + '&status=' + full.transaksi_status + '"> Approved </a>'
            } else if (full.transaksi_status == '5' && ($('#user_unit_id').val() == 'E44000' && $('#user_poscode').val() == 'E44150051A')) {
              return '<a class="btn btn-success" href="<?= base_url('sample/lab/procesLab/?header_menu=' . $_GET['header_menu'] . '&menu_id=' . $_GET['menu_id'] . '&non_rutin=') ?>' + full.id_transaksi_non_rutin + '&status=' + full.transaksi_status + '"> Disposisi </a>'
            } else {
              return '-'
            }
          }
  ...
  /* FIlter */
  $('#filter').on('submit', function(e) { //button cari item
    e.preventDefault();
    $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
      if (!json.user_id) {
        fun_notifLogout();
      } else {
        $('#table').DataTable().ajax.url('<?= base_url() ?>sample/lab/getLab?' + $('#filter').serialize()).load();
      }
    })
  })
  /* FIlter */
