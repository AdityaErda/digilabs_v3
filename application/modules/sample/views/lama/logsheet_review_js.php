<script>
  ...
  $('#send_dof').on('click', function() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        // url = '<?= base_url('sample/inbox/coba_word') ?>';
  ...
  tinymce.init({
    ...
    width: '210mm',
    height:'297mm',
    content_css: '<?=base_url()?>assets_tambahan/tinymce/custom.css',
    script_url: '<?= base_url() ?>assets_tambahan/tinymce/tinymce.min.js', // replace with your own path
    // height: 300,
    plugins: "advlist code searchreplace autolink directionality visualblocks visualchars image link media codesample table charmap pagebreak nonbreaking anchor insertdatetime advlist charmap emoticons print autosave autoresize charmap colorpicker contextmenu fullscreen fullpage textpattern",
  ...
  function func_ganti_bahasa(id) {
    ...
    $('#id_template_footer').select2({
      ...
      ajax: {
        delay: 250,
        url: '<?= base_url('master/footer_sertifikat/getFooterSertifikatList?bahasa=') ?>' + id,
        dataType: 'json',
        type: 'GET',
        data: function(params) {
          var queryParameters = {
