  ...
  $(function() {
    ...
    $('#table').DataTable({
      ...
      },
      {
        ...
        {
          "render": function(data, type, full, meta) {
            return '<center><a href="javascript:;" id="' + full.transaksi_rutin_id + '" title="Detail" onclick="fun_detail(this.id)"><i class="fa fa-search"></i></a></center>';
          }
        },
        {
          "render": function(data, type, full, meta) {
        ...
        },
        {
          "render": function(data, type, full, meta) {
            ...
            // return (full.status > '0') ? '' : '<center><a href="javascript:;" id="' + full.transaksi_rutin_id + '" title="Proses" onclick="fun_cara_close(this.id,`' + full.status + '`);fun_close_nama(`y`);"><i class="fa fa-share" data-toggle="modal" data-target="#modal_cara_close" style="color: green"></i></a></center>';
            // return (full.status > '0') ? '' : '<center><a href="javascript:;" id="' + full.transaksi_rutin_id + '" title="Proses" onclick="fun_proses(this.id)"><i class="fa fa-share" data-toggle="modal" data-target="#modal_proses" style="color: green"></i></a></center>';
          }
        },
        ]
});
...
$('#table_detail').DataTable({
  ...
  {
    "render": function(data, type, full, meta) {
      return (full.transaksi_detail_status == '0') ? '<center><a href="javascript:;" id="' + full.transaksi_id + '" title="Proses" onclick="fun_proses_detail(this.id)" data-target="#modal_proses" data-toggle="modal"><i class="fa fa-share"  style="color: green"></i></a></center>' : '-';
    }
  },
  {
    "render": function(data, type, full, meta) {
...
function fun_detail(id) {
  $.getJSON('<?= base_url('login/login/checkLogin') ?>', {}, function(json) {
    if (!json.user_id) {
      ...
      $('#table_detail').DataTable().ajax.url('<?= base_url() ?>sample/nomor/getNomorDetail?&transaksi_rutin_id=' + id).load();
      $('html, body').animate({
...
function fun_hapus(id, value) {
  $.confirmModal('Apakah Anda Yakin Akan Menghapus Data Ini ?', function(e) {
    $.getJSON('<?= base_url('sample/nomor/hapusNomorDetail') ?>', {
      transaksi_id: id,
