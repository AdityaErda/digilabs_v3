<script type="text/javascript">
  ...
  $('#add_item').on('click', function() {
    ...
    html += '<div class="col-8">';
    html += '<input type="file" name="transaksi_detail_file[]" id="transaksi_detail_file_' + urut + '" class="form-control" accept="image/*" required>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
  ...
  $('#simpan_approved').on('click', function(e) {
    e.preventDefault();

    var set_data = new FormData($('#form_approved')[0]);
    set_data.append('transaksi_agreement_keterangan', $('#transaksi_agreement_keterangan').val());

    var url = '<?= base_url('sample/approved/insertApproved') ?>';
