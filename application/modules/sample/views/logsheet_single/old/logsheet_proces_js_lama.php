<script>
  ...
  function fun_rumus(id) {
    ...
    $.getJSON('<?= base_url() ?>/master/template_logsheet/getDetailLogsheet', {
      ...
      $.each(json, function(index, val) {
        ...
        html += '</table>';
        html += '</div>';
        fun_detail_rumus(val.rumus_id,val.satuan_sample);
  ...
  function fun_detail_rumus(id,satuan) {
    // var simplo = "";
    var header = "";
    var body = "";
    var footer = "";
    ...
    $.getJSON('<?= base_url() ?>/master/perhitungan_sample/getDetailRumusSampleTemplate', {
      ...
      body += '\
      <td><input type="text" id="rumus_metoda_'+id+'" name="rumus_metoda[]" value="'+satuan+'" class="form-control rumus_metoda"></td>\
      ...
      <td width="20px">\
      <a href="javascript:void(0);" id="' + id + '" onclick="add_simplo(this.id,`'+satuan+'`)"><i class="fa fa-plus" style="color:green"></i></a>\
      <br>\
      </td></tr>';
...
function add_simplo(id,satuan) {
  ...
  $.getJSON('<?= base_url() ?>/master/perhitungan_sample/getDetailRumusSampleTemplate', {
    ...
    header += '<th>Satuan</th><th>Hasil</th><th>Aksi</th></tr>';
    body += '\
