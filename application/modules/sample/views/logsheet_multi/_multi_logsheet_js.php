<script>
    ...
    // render detail log column
		...
		$('#logsheet_pengolah_sample').select2({
			...
			ajax: {
				delay: 250,
				url: '<?= base_url('master/jenis_sample_uji/getPengambilList') ?>',
				dataType: 'json',
				type: 'GET',
	...
	function notif_simpan(){
		...
		Swal.fire({
			...
			if (result.value) {
				...
				// 	url = '<?= base_url('sample/multi_sample/insertDraftLogSheet') ?>';
				// 	$.ajax({
				// 		type: "POST",
				// 		url: url,
				// 		data: data,
				...
				// 			location.href = '<?= base_url('sample/multi_sample/draftLogSheet/?header_menu=' . $_GET['header_menu'] . '&menu_id=' . $_GET['menu_id']) . '&transaksi_id=' . $_GET['transaksi_id'] ?>' + response;
				// 		}
				// 	});
			}
		})
...
/* Kembali */
function kembali_multiple() {
	Swal.fire({
		title: "Kembali Ke Multiple?",
		text: "Apakah Anda Yakin Kembali Ke Halaman Utama Multiple ?",
		icon: "danger",
		showCancelButton: true,
		confirmButtonColor: "#34c38f",
		cancelButtonColor: "#f46a6a",
